    ...
    return (
      <div className="pt-32 pb-24 max-w-md mx-auto px-4 animate-fade-in text-center">
        <div className="bg-slate-900 border border-slate-800 p-10 rounded-[2.5rem] shadow-2xl">
          ...
          <p className="text-slate-400 text-[10px] uppercase mb-8">Vérifiez vos emails.</p>
      ...
      <div className="bg-slate-900 border border-slate-800 p-10 md:p-20 rounded-[4rem] shadow-2xl relative overflow-hidden">
        <div className="text-center mb-16 relative z-10">
          <h1 className="text-6xl md:text-8xl font-gaming font-black text-white uppercase tracking-tighter mb-4 leading-none">
            MoonNight <br />
            <span className="text-sky-400">Shop</span>
          </h1>
        ...
        <div className="flex bg-slate-950 p-2 rounded-[2.5rem] mb-12 border border-slate-800 relative z-10">
          <button onClick={() => {setMode('login'); setError(null);}} className={`flex-1 py-5 rounded-[2rem] text-[11px] font-gaming uppercase transition-all duration-300 ${mode === 'login' ? 'bg-sky-500 text-white' : 'text-slate-500 hover:text-white'}`}>Connexion</button>
          <button onClick={() => {setMode('signup'); setError(null);}} className={`flex-1 py-5 rounded-[2rem] text-[11px] font-gaming uppercase transition-all duration-300 ${mode === 'signup' ? 'bg-sky-500 text-white' : 'text-slate-500 hover:text-white'}`}>Inscription</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          ...
          <div className="animate-slide-up">
            <label className="block text-slate-500 text-[10px] font-gaming uppercase mb-2 ml-4">Email</label>
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-sky-500 text-white font-gaming font-bold py-6 rounded-[2rem] shadow-xl hover:bg-sky-600 transition-all uppercase tracking-[0.3em] mt-10">
